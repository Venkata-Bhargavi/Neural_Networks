...
class Layer:
    ...
    def forward_propagation(self, inputs, targets, training=True):
        if self.normalization_type == "z_score":
            ...
            inputs_normalized, mean, std_dev = z_score_normalization(inputs)
        elif self.normalization_type == "min_max":
            # Apply Min-Max normalization
            inputs_normalized, min_val, max_val = min_max_normalization(inputs)
        else:
            inputs_normalized = inputs  # No normalization
        ...
        for neuron in self.neurons:
            ...
            neuron.layer = self
            if training:
                inputs_normalized = self.dropout.forward_propagation(inputs_normalized)
            neuron.neuron(inputs_normalized)
